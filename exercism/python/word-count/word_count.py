...
def words(phrase: str) -> list:
    raise _coconut_tail_call(re.findall, r'[^\W_]+', phrase)

def word_set(phrase: str) -> set:
    return ((set)(words(phrase)))

def word_count(phrase: str) -> dict:
    word_counter = lambda word: words(phrase.lower()).count(word)
    return {word: word_counter(word) for word in word_set(phrase.lower())}
