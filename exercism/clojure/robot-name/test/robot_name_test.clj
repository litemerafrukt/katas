...
(defn reset-robot-name-pool [test]
  (robot-name/reset-name-pool)
  (time (test)))

(use-fixtures :each reset-robot-name-pool)
...
(deftest unique-names
  (let [total-number-of-names (* 26 26 10 10 10)
        all-names (->> (repeatedly #(robot-name/robot-name (robot-name/robot)))
                       (take total-number-of-names)
                       (into #{}))]

    (testing "produce all unique names"
      (is (= (count all-names) total-number-of-names)))))
